function [  demod,BER  ] = QPSK( input_bit_stream,Eb,No,jammer_flag,jammer_energy )
a = sqrt(Eb/2);
%% QPSK modulation
output = zeros(2,length(input_bit_stream));
noise = sqrt(No/2)*randn(2,length(input_bit_stream));
for i = 1:2:length(input_bit_stream)
    if input_bit_stream(i) == 0 && input_bit_stream(i+1) == 0
        output(1,i) = -a;
        output(2,i) = -a;   
    end
    if input_bit_stream(i) == 0 && input_bit_stream(i+1) == 1
        output(1,i) = -a;
        output(2,i) =  a;   
    end
    ...
    if input_bit_stream(i) == 1 && input_bit_stream(i+1) == 0
        output(1,i) =  a;
        output(2,i) = -a;   
    end
end
